...
-- SQL Server compatible CREATE TABLE IF NOT EXISTS statements
...
IF NOT EXISTS (
    SELECT 1
    FROM sys.tables
    WHERE name = 'DriverProfiles' AND schema_id = SCHEMA_ID('dbo')
)
BEGIN
CREATE TABLE dbo.DriverProfiles (
                                    driverId   INT         NOT NULL PRIMARY KEY,
                                    shiftTime  NVARCHAR(50) NULL,
                                    CONSTRAINT FK_DriverProfiles_Drivers
...
IF NOT EXISTS (
    SELECT 1
    FROM sys.tables
    WHERE name = 'DeliveryTimeline' AND schema_id = SCHEMA_ID('dbo')
)
BEGIN
CREATE TABLE dbo.DeliveryTimeline (
                                      deliveryId     INT          NOT NULL PRIMARY KEY,
                                      assignedTime   DATETIME2    NULL,
                                      pickedUpTime   DATETIME2    NULL,
                                      inTransitTime  DATETIME2    NULL,
                                      completedTime  DATETIME2    NULL,
                                      eta            NVARCHAR(50) NULL,
                                      CONSTRAINT FK_DeliveryTimeline_Deliveries
...
IF NOT EXISTS (
    SELECT 1
    FROM sys.tables
    WHERE name = 'DeliveryProofs' AND schema_id = SCHEMA_ID('dbo')
)
BEGIN
CREATE TABLE dbo.DeliveryProofs (
                                    ...
                                    notes          NVARCHAR(255) NULL,
                                    proofPhotoUrl  NVARCHAR(255) NULL,
                                    signaturePath  NVARCHAR(255) NULL,
                                    createdAt      DATETIME2     NOT NULL DEFAULT SYSUTCDATETIME(),
                                    CONSTRAINT FK_DeliveryProofs_Deliveries
                                        FOREIGN KEY (deliveryId) REFERENCES dbo.Deliveries(id)
);
END
GO
...
IF NOT EXISTS (
    SELECT 1
    FROM sys.tables
    WHERE name = 'DeliveryIssues' AND schema_id = SCHEMA_ID('dbo')
)
BEGIN
CREATE TABLE dbo.DeliveryIssues (
                                    ...
                                    createdAt    DATETIME2     NOT NULL DEFAULT SYSUTCDATETIME(),
                                    CONSTRAINT FK_DeliveryIssues_Deliveries
                                        FOREIGN KEY (deliveryId) REFERENCES dbo.Deliveries(id)
);
CREATE INDEX IX_DeliveryIssues_Delivery ON dbo.DeliveryIssues(deliveryId);
END
GO
...
/* Optional: backfill timeline rows for currently active deliveries -------- */
;WITH s AS (
...
USING s ON T.deliveryId = s.deliveryId

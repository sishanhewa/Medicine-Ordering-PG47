-- Migration script to add customer support tables
-- Run this script to add the missing tables to your existing database

-- Table 1: Messages (Customer Support)
...
-- Table 3: Issues (Customer Support)
...
-- Add some test data
IF NOT EXISTS (SELECT 1 FROM dbo.Messages WHERE sender = 'customer1@test.com')
BEGIN
...
IF NOT EXISTS (SELECT 1 FROM dbo.Notifications WHERE recipient = 'customer1@test.com')
BEGIN
...
PRINT 'Customer support tables migration completed successfully!';
